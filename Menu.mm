...
UIColor *glowColor = UIColorFromHex(0x00158a); // xanh dương

@interface Menu ()

@property (assign, nonatomic) CGPoint lastMenuLocation;
@property (strong, nonatomic) UILabel *menuTitle;
@property (strong, nonatomic) UILabel *menuCre;
...
-(id)initWithTitle:(NSString *)title_ titleColor:(UIColor *)titleColor_ titleFont:(NSString *)titleFont_ credits:(NSString *)credits_ headerColor:(UIColor *)headerColor_ switchOffColor:(UIColor *)switchOffColor_ switchOnColor:(UIColor *)switchOnColor_ switchTitleFont:(NSString *)switchTitleFont_ switchTitleColor:(UIColor *)switchTitleColor_ infoButtonColor:(UIColor *)infoButtonColor_ maxVisibleSwitches:(int)maxVisibleSwitches_ menuWidth:(CGFloat )menuWidth_ menuIcon:(NSString *)menuIconBase64_ menuButton:(NSString *)menuButtonBase64_ {
    ...
    // Base of the Menu UI.
    self = [super initWithFrame:CGRectMake(0,0,menuWidth_, maxVisibleSwitches_ * 50 + 40)];
    ...
    self.layer.borderColor = [UIColor colorWithRed:0.00 green:0.00 blue:1.00 alpha:1.0].CGColor;
   ...
   // self.header.layer.mask = headerLayer;
    ...
    self.menuTitle = [[UILabel alloc]initWithFrame:CGRectMake( menuWidth_ /2/2/2, -2, menuWidth_ - 60, 40)];
    self.menuTitle.text = title_;
    self.menuTitle.textColor = titleColor_;
    self.menuTitle.font = [UIFont fontWithName:titleFont_ size:25.0f];
    ...
    self.menuCre = [[UILabel alloc]initWithFrame:CGRectMake( menuWidth_ /2/2/2, -2, menuWidth_ - 65, 20)];
    self.menuCre.text = @"мᴀᴅᴇ ғoʀ κᴇɴнтᴀo.ɴᴇт";
    self.menuCre.textColor = titleColor_;
    self.menuCre.font = [UIFont fontWithName:titleFont_ size:10.0f];
    self.menuCre.adjustsFontSizeToFitWidth = true;
    self.menuCre.textAlignment = NSTextAlignmentCenter;
    [self.footer addSubview: self.menuCre];
...
- (id)initHackNamed:(NSString *)hackName_ description:(NSString *)description_ offsets:(std::vector<uint64_t>)offsets_ bytes:(std::vector<std::string>)bytes_ {
 ...
 UIAlertController *sucsses = [UIAlertController alertControllerWithTitle:@"Số lượng đầu vào không hợp lệ" message:[NSString stringWithFormat:@"Offsets Mảng đầu vào (%d) không bằng số đầu vào mảng byte (%d)", (int)offsets_.size(), (int)bytes_.size()] preferredStyle:UIAlertControllerStyleAlert];
 ...
 UIAlertController *sucsses = [UIAlertController alertControllerWithTitle:@"Số lượng đầu vào không hợp lệ" message:[NSString stringWithFormat:@"Offsets Mảng đầu vào (%d) không bằng số đầu vào mảng byte (%d)", (int)offsets_.size(), (int)bytes_.size()] preferredStyle:UIAlertControllerStyleAlert];
    ...
    self.layer.borderColor = [UIColor colorWithRed:0.40 green:0.40 blue:0.40 alpha:1.0].CGColor;

    switchLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, menuWidth, 35)];
    switchLabel.text = hackName_;
    switchLabel.textColor = switchTitleColor;
    switchLabel.font = [UIFont fontWithName:switchTitleFont size:15];
    switchLabel.adjustsFontSizeToFitWidth = true;
    switchLabel.textAlignment = NSTextAlignmentLeft;
    [self addSubview:switchLabel];

    UIButton *infoButton = [UIButton buttonWithType:UIButtonTypeInfoDark];
    infoButton.frame = CGRectMake(menuWidth - 30, 10, 15, 15);
...
- (id)initTextfieldNamed:(NSString *)hackName_ description:(NSString *)description_ inputBorderColor:(UIColor *)inputBorderColor_ {
    ...
    switchLabel.text = hackName_;
    switchLabel.textColor = switchTitleColor;
    switchLabel.font = [UIFont fontWithName:switchTitleFont size:8];
    switchLabel.adjustsFontSizeToFitWidth = true;
    switchLabel.textAlignment = NSTextAlignmentLeft;
    [self addSubview:switchLabel];

    textfieldValue = [[UITextField alloc]initWithFrame:CGRectMake(menuWidth / 2 + 20, 8, menuWidth / 3, 20)];
    ...
    textfieldValue.font = [UIFont boldSystemFontOfSize:13];
    textfieldValue.borderStyle = UITextBorderStyleLine;
    textfieldValue.textAlignment = NSTextAlignmentLeft;
...
- (id)initSliderNamed:(NSString *)hackName_ description:(NSString *)description_ minimumValue:(float)minimumValue_ maximumValue:(float)maximumValue_ sliderColor:(UIColor *)sliderColor_{
    ...
    sliderValue = [[UISlider alloc]initWithFrame:CGRectMake(menuWidth / 2, 8, menuWidth / 3 + 50, 20)];
    sliderValue.thumbTintColor = sliderColor_;
    sliderValue.minimumTrackTintColor = switchTitleColor;
    sliderValue.transform = CGAffineTransformMakeScale(0.6, 0.6);
